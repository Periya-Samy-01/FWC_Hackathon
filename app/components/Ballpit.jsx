...
class Z extends d {
  ...
  setColors(e) {
    if (Array.isArray(e) && e.length > 0) {
      const colorInstances = e.map(c => new l(c));
      const white = new l(0xffffff);

      this.setColorAt(0, white);
      this.light.color.copy(white);

      for (let idx = 1; idx < this.config.count; idx++) {
        const rand = Math.random();
        let color;
        if (colorInstances.length >= 3) {
            if (rand < 0.4) {
              color = colorInstances[0];
            } else if (rand < 0.8) {
              color = colorInstances[1];
            } else {
              color = colorInstances[2];
            }
        } else {
            color = colorInstances[0];
        }
