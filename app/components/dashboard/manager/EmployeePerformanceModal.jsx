...
const SkillsTab = ({ employee }) => {
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      // API endpoint is now unified
      const [skillsRes, masterSkillsRes] = await Promise.all([
        fetch(`/api/employee-skills?employeeId=${employee._id}`),
        fetch('/api/skills')
      ]);

      if (!skillsRes.ok) throw new Error('Failed to fetch skill data.');
      if (!masterSkillsRes.ok) throw new Error('Failed to fetch master skills.');

      const skillsData = await skillsRes.json();
      const masterSkillsData = await masterSkillsRes.json();

      setSkillData(Array.isArray(skillsData) ? skillsData : []);
  ...
  const handleSaveSkill = async (e) => {
    ...
    try {
      ...
      setSkillData(updatedSkills); // Instantly update chart with response
      setSelectedSkill('');
  ...
  const trainingResources = {
    "JavaScript": "Advanced JS course on Udemy",
    "React": "Official React documentation and tutorials",
    "Node.js": "Node.js design patterns book",
    "Project Management": "Agile and Scrum certification",
    "Leadership": "Internal management training program"
  };

  const getTrainingRecommendation = (skill) => {
    return trainingResources[skill] || "Explore general resources for this skill.";
  };
  ...
  const chartData = skillData.map(d => ({
    skill: d.skill,
    required: proficiencyToValue[d.requiredProficiency],
    current: proficiencyToValue[d.currentProficiency],
  }));

  const skillGaps = skillData.filter(d =>
    proficiencyToValue[d.currentProficiency] < proficiencyToValue[d.requiredProficiency]
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div>
        ...
        <div className="mt-6">
          <h4 className="font-bold mb-2">Training Recommendations</h4>
          {skillGaps.length > 0 ? (
            <ul className="list-disc pl-5 space-y-1">
              {skillGaps.map(gap => (
                <li key={gap.skill}>
                  <strong>{gap.skill}:</strong> {getTrainingRecommendation(gap.skill)}
                </li>
              ))}
            </ul>
      ...
      <div>
        <h3 className="text-xl font-bold mb-4">Manual Skill Assessment</h3>
        <form onSubmit={handleSaveSkill} className="bg-gray-50 p-4 rounded-lg border">
          <div className="mb-4">
            <label htmlFor="skill-select" className="block text-sm font-medium text-gray-700 mb-1">Skill</label>
            <select
              id="skill-select"
              value={selectedSkill}
          ...
          <div className="mb-4">
            <label htmlFor="proficiency-select" className="block text-sm font-medium text-gray-700 mb-1">Proficiency Level</label>
            <select
          ...
          <button type="submit" className="w-full bg-blue-500 text-white py-2 rounded-md hover:bg-blue-600 transition-colors">Save Skill</button>
        </form>
      </div>
    </div>
  );
